...
export function Topbar({ activeSection, onNavigate }: TopbarProps) {
  return (
    <nav
      className="flex flex-wrap items-center gap-1 rounded-xl border border-border/80 bg-card/40 p-1"
      ...
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <button
            ...
            className="relative flex flex-1 min-w-[88px] items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[13px] font-medium font-inter transition-colors duration-300 cursor-pointer border-none bg-transparent"
          >
            {isActive && (
              <motion.span
                layoutId="activeTab"
