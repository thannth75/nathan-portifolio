...
export function StatusBar({ activeSection, scrollProgress }: StatusBarProps) {
  ...
  return (
    <div className="shrink-0 border-t border-border bg-card/30 px-5 py-2.5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)] shrink-0" />
          <span className="font-mono text-[10px] sm:text-[11px] text-muted/70 truncate">
            nathan@portfolio ~ {SECTION_NAMES[activeSection].toLowerCase()}
          </span>
        </div>

        <div className="hidden md:block font-mono text-[10px] text-muted/40 shrink-0">
          {time} · São Carlos
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <span className="font-mono text-[10px] text-muted/50 w-8 text-right">
